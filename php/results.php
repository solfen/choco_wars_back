<?php
	...
	$request = $connexion->prepare("SELECT `Round`,`Turnover`, `Earnings`, `Name` FROM `chocowars_teamresults` JOIN `chocowars_teams` ON `TeamID` = `chocowars_teams`.`ID` WHERE `Turnover` != 0 || `Earnings` != 0 ORDER BY `Name`");
	$request->execute();
